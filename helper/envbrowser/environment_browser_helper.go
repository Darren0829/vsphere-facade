...
func New(c *vim25.Client, ref types.ManagedObjectReference) *EnvironmentBrowser {
...
func (b *EnvironmentBrowser) GetOSFamilies(ctx context.Context) []types.GuestOsDescriptor {
	var eb mo.EnvironmentBrowser
	err := b.Properties(ctx, b.Reference(), nil, &eb)
	if err != nil {
		return nil
	...
	if res.Returnval == nil {
		logging.L().Error("", errors.New("no config options were found for the supplied criteria"))
		return nil
	}
	return res.Returnval.GuestOSDescriptor
}
